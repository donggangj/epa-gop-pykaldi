...
def get_reference_from_system_alignments(reference_transcriptions_path, labels_dir_path, system_alignments,
                                         utterance_list):
    output = {}

    # Load reference dictionaries from reference transcriptions
    ref_dict_with_zeros, ref_dict_without_zeros, sent_dict_complete = generate_dict_from_transcripctions(
        reference_transcriptions_path)
    ...
    for utterance in utterance_list:
        ...
        best_ref_auto, best_ref_auto_zero = find_best_ref(trans_auto, ref_dict_with_zeros[sent],
                                                          ref_dict_without_zeros[sent], spk)

        print("BEST_REF_AUTO:        %s (chosen out of %d transcriptions)" % (
        phonelist2str(best_ref_auto), len(ref_dict_without_zeros[sent])))
