...
def read_value_avg(file, layer_number, is_tdnnf=True):
    layer_type = get_layer_type(is_tdnnf)
    return read_vector(file, '<ComponentName> ' + layer_type + str(
        layer_number) + '.relu <RectifiedLinearComponent> <Dim> 1536 <ValueAvg>  [ ', False)
...
def read_stats_mean(file, layer_number, is_tdnnf=True, component_name='', dim='1536'):
    ...
    return read_vector(file,
                       '<ComponentName> ' + component_name + ' <BatchNormComponent> <Dim> ' + dim + ' <BlockDim> ' + dim + ' <Epsilon> 0.001 <TargetRms> 1 <TestMode> F <Count> ' + count + ' <StatsMean>  [ ',
                       False)
...
def read_affine_component(file):
    params_dict = {'linear_params': read_linear_params(file), 'bias': read_bias(file)}
    return params_dict


def read_batchnorm_component(file, layer_number, is_tdnnf=True, component_name='', dim='1536'):
    params_dict = {'stats_mean': read_stats_mean(file, layer_number, is_tdnnf=is_tdnnf, component_name=component_name,
                                                 dim=dim), 'stats_var': read_stats_var(file)}
    return params_dict
...
def main(config_dict):
    ...
    while not finished:
        ...
        if '<ComponentName> prefinal-chain.batchnorm1' in line:
            components['prefinal-chain.batchnorm1'] = read_batchnorm_component(
                chain_file, 19,
                component_name='prefinal-chain.batchnorm1')

        if '<ComponentName> prefinal-chain.batchnorm2' in line:
            components['prefinal-chain.batchnorm2'] = read_batchnorm_component(
                chain_file, 19,
                component_name='prefinal-chain.batchnorm2',
                dim='256')
    ...
    state_dict = {'ftdnn.layer01.lda.weight': torch.from_numpy(components['lda']['linear_params']),
                  'ftdnn.layer01.lda.bias': torch.from_numpy(components['lda']['bias']),
                  'ftdnn.layer01.kernel.weight': torch.from_numpy(components['tdnn1.affine']['linear_params']),
                  'ftdnn.layer01.kernel.bias': torch.from_numpy(components['tdnn1.affine']['bias']),
                  'ftdnn.layer01.bn.running_mean': torch.from_numpy(components['tdnn1.batchnorm']['stats_mean']),
                  'ftdnn.layer01.bn.running_var': torch.from_numpy(components['tdnn1.batchnorm']['stats_var'])}
    ...
    state_dict['output_layer.bn1.running_mean'] = torch.from_numpy(
        components['prefinal-chain.batchnorm1']['stats_mean'])
    ...
    state_dict['output_layer.bn2.running_mean'] = torch.from_numpy(
        components['prefinal-chain.batchnorm2']['stats_mean'])
