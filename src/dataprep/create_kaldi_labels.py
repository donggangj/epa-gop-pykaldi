...
def match_trans_lengths(trans_dict, start_times, end_times):
    ...
    if len(trans_auto) != len(trans_manual):
        ...
        trans_manual, trans_auto, labels, start_times, end_times = remove_deletion_lines(trans_manual, trans_auto,
                                                                                         labels,
                                                                                         remove_times=True,
...
def remove_deletion_lines(trans1, trans2, labels, remove_times=False, start_times=None, end_times=None):
    ...
    if remove_times and (start_times is None or end_times is None):
        raise Exception('remove_times is True but start or end times are missing')
    if not remove_times and (start_times is not None or end_times is not None):
    ...
    if remove_times:
        ...
        trans1, trans2, labels, _, _ = remove_deletion_lines_with_times(trans1, trans2, labels, range(len(trans1)),
                                                                        range(len(trans1)))
...
def main(config_dict):
    ...
    trans_dict = get_reference_from_system_alignments(reference_transcriptions_path, labels_dir_path, kaldi_alignments,
                                                      utterance_list)
    ...
    for utterance in utterance_list:
        ...
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        try:
            np.savetxt(outfile, np.c_[
                np.arange(len(target_column)), target_column, trans_manual, labels, start_times, end_times],
                       fmt=utterance + "_%s %s %s %s %s %s")
