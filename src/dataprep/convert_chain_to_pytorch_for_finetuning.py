...
def main(config_dict):
    ...
    for layer_number in range(2, 18):
        model_state_dict['ftdnn.layer' + str("{:02d}".format(layer_number)) + '.sorth.weight'] = torch.from_numpy(
            components['tdnnf' + str(layer_number) + '.linear']['linear_params'])
        model_state_dict['ftdnn.layer' + str("{:02d}".format(layer_number)) + '.affine.weight'] = torch.from_numpy(
            components['tdnnf' + str(layer_number) + '.affine']['linear_params'])
        model_state_dict['ftdnn.layer' + str("{:02d}".format(layer_number)) + '.affine.bias'] = torch.from_numpy(
            components['tdnnf' + str(layer_number) + '.affine']['bias'])
        model_state_dict['ftdnn.layer' + str("{:02d}".format(layer_number)) + '.sorth.weight'] = torch.from_numpy(
            components['tdnnf' + str(layer_number) + '.linear']['linear_params'])
        model_state_dict['ftdnn.layer' + str("{:02d}".format(layer_number)) + '.bn.running_mean'] = torch.from_numpy(
            components['tdnnf' + str(layer_number) + '.batchnorm']['stats_mean'])
        model_state_dict['ftdnn.layer' + str("{:02d}".format(layer_number)) + '.bn.running_var'] = torch.from_numpy(
