...
def load_data_for_eval(load_path):
    df = joblib.load(load_path)
...
def compare_models(dataframes, labels, output, colors):
    ...
    for phidx, phone in enumerate(phones_to_plot):
        ...
        labels_non_target = [list(df.loc[(df['phones_names'] == phone) & (df['label'] == 0)].label) for df in
                             dataframes]

        all_samples = [targets[i] + non_targets[i] for i in range(systems_count)]

        rocs = [roc_curve(labels_target[i] + labels_non_target[i], targets[i] + non_targets[i]) for i in
                range(systems_count)]
        ...
        eers = [brentq(lambda x: 1. - x - interpolate.interp1d(fprs[i], tprs[i])(x), 0., 1.) for i, _ in
                enumerate(fprs)]
        ...
        for i in range(systems_count):
            legend = labels[i] + ' (AUC = %0.2f ' % roc_aucs[i] + ' EER =  %0.2f ' % eers[i] + ' cant+ =  %0.2i ' % len(
                targets[i]) + ' cant- =  %0.2i ' % len(non_targets[i]) + ')'
