...
def plot(scores, output_dir):
    ...
    for phone in phones:
        ...
        if len(pos_scores) >= 50 and len(neg_scores) >= 50:
            ...
            print("Plotting phone %2s, AUC = %.2f, EER = %.2f, num_correct = %d, num_incorrect = %d" % (
            phone, roc_auc, eer, len(neg_scores), len(pos_scores)))
            ...
            print("Not plotting phone %2s, due to too few samples, num_correct = %d, num_incorrect = %d" % (
            phone, len(neg_scores), len(pos_scores)))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()

    parser.add_argument('--data-for-eval-dir', dest='data_for_eval', help='Dir where data_for_eval.pickle is',
                        default=None)
