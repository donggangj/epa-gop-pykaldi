...
def criterion_fast(batch_outputs, batch_labels, weights=None, norm_per_phone_and_class=False,
                   log_per_phone_and_class_loss=False, phone_int2sym=None, phone_int2node=None, min_frame_count=0):
    ...
    loss_pos, sum_weights_pos = calculate_loss(batch_outputs, batch_labels == 1, batch_labels_for_loss,
                                               phone_weights=weights, norm_per_phone_and_class=norm_per_phone_and_class,
...
def foward_backward_pass(data, model, optimizer, phone_weights, phone_int2sym, phone_int2node,
    ...
    loss = criterion_fast(outputs, batch_labels, weights=phone_weights, phone_int2sym=phone_int2sym,
                          phone_int2node=phone_int2node, norm_per_phone_and_class=norm_per_phone_and_class,
                          min_frame_count=0)
...
def train_one_epoch(trainloader, testloader, model, optimizer, running_loss, fold, epoch, step, use_clipping,
                    phone_weights, phone_int2sym, phone_int2node, norm_per_phone_and_class):
...
def train(model, trainloader, testloader, fold, epochs, swa_epochs, state_dict_dir, run_name, layer_amount, use_dropout,
          lr, scheduler_config, swa_lr, use_clipping, batchnorm, norm_per_phone_and_class):
    ...
    for epoch in range(start_from_epoch, epochs):  # loop over the dataset multiple times
        ...
        running_loss, step, model, optimizer = train_one_epoch(trainloader, testloader, model, optimizer, running_loss,
                                                               fold, epoch,
                                                               step, use_clipping, phone_weights, phone_int2sym,
                                                               phone_int2node, norm_per_phone_and_class)
        ...
        if swa_epochs > 0 and epoch >= swa_start:
            ...
            if epoch % checkpoint_step == checkpoint_step - 1:
                save_state_dict(state_dict_dir, run_name, fold, epoch + 1, step, swa_model, optimizer, scheduler,
                                suffix='_swa')
